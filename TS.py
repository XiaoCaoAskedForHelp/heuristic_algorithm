...
class TS(object):
    ...
    def ts_search(self, x):
        moves = []
        new_paths = []
        while len(new_paths) < 400:  # 形成400个候选集
    ...
    def ts(self):
        for cnt in range(self.iteration):
            ...
            if min_l < self.best_length:
                ...
                # 更新禁忌表
                if min_move in self.taboo:   # 如果当前解是最优解，则将其解禁
                    self.taboo.remove(min_move)

                self.taboo.append(min_move)
            else:
                # 找到不在禁忌表中的操作
                while min_move in self.taboo:  # 如果min_move在禁忌表中，则一直向下寻找最优解
                    sort_index = sort_index[1:]   # 找到次优解
                    min_path = new_paths[sort_index[0]]
                    min_move = moves[sort_index[0]]
                self.cur_path = min_path # 最终得到一个不在表中的解
                self.taboo.append(min_move)
            # 禁忌表超长了
...
axs[1].scatter(Best_path[:, 0], Best_path[:, 1])
Best_path = np.vstack([Best_path, Best_path[0]])
axs[1].plot(Best_path[:, 0], Best_path[:, 1])
axs[1].set_title('规划结果')
iterations = model.iter_x
best_record = model.iter_y
axs[2].plot(iterations, best_record)
axs[2].set_title('收敛曲线')
plt.show()
