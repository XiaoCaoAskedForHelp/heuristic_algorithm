...
class ACO(object):
    def __init__(self, num_city, data, iteration=500):
        self.m = 50  # 蚂蚁数量
        ...
        self.Tau = np.ones([num_city, num_city])  # 可能更小会更好，因为添加的信息素都是小数点后两位的量级
        self.Table = [[0 for _ in range(num_city)] for _ in range(self.m)]  # 生成的蚁群  50 * 70
        self.iter = 1
...
aco = ACO(num_city=data.shape[0], data=data.copy(), iteration=500)
...
axs[0].set_title('st70:蚁群算法规划结果')

iterations = range(aco.iter_max)
